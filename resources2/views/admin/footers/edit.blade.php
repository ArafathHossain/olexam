...
<div class="content-header row align-items-center m-0">
    <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
        <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
            ...
            <li class="breadcrumb-item active">Update Footer Widget</li>
        </ol>
    </nav>
    <div class="col-sm-8 header-title p-0">
        <div class="media">
            ...
            <div class="media-body">
                <h1 class="font-weight-bold">Update Footer Widget</h1>
...
<div class="card mb-4">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h6 class="fs-17 font-weight-600 mb-0">Update Footer Widget</h6>
            </div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.footers.update', $footer->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="column"
                                ...
                                <option value="{{ $column }}" {{ $footer->column == $column  ? 'selected' : '' }}>
                                    {{ word_view($column) }}
                                </option>
                                @endforeach
                            </select>
                ...
                @php
                $menus = explode('||',$footer->name);
                $links = explode('||',$footer->link);
                $items = array_combine($menus, $links);
                @endphp
                ...
                <div class="row menu_item_clone">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="link" class="font-weight-600">Menu Link</label>
                            ...
                            <select class="form-control {{ $errors->has('link.*') ? 'is-invalid' : '' }}" name="link[]">
                                @foreach(site_links() as $li => $name)
                                <option value="{{ $li }}" {{ $link == $li ? 'selected' : ''}}>
                                    {{ word_view($name) }}
                                </option>
                                @endforeach
                                @foreach($pages as $li)
                                <option value="/page/{{ $li->slug }}"
                                    {{ $link == '/page/' .$li->slug  ? 'selected' : '' }}>
                                    {{ word_view($li->title) }}
                                </option>
                                @endforeach
                            </select>
...
<script>
    ...
    $('.menu_item_clone').not(":first").find('.delete_btn').append(
