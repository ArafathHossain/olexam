...
class FrontendController extends Controller
{
        ...
        $enrolls = DB::table('packages as p')
            ...
            ->where(function ($query) use ($auth) {
                $query->where('r.status', 'Complete')
                    ...
                    ->where('p.id', '!=', 'mua.package_id')
                    // ->where('mua.user_id', '!=', $auth)
                ;
            })
            ->distinct('p.id')
            ->select('p.*')
    ...
    {
        $enrolls = Enroll::orderBy('id', 'desc')->where('status', 'Complete')->where('user_id', auth()->id())->get();
        return view('frontend.all_courses', compact('enrolls'));
    }
    public function my_orders()
    {
        $orders = Enroll::where('user_id', auth()->id())->get();
